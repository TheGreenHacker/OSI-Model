...
void test_get_node_fns(graph_t *graph) {
    node_t *n1 = get_node_by_node_name(graph, "R0_re");
    assert(n1 && get_node_if_by_name(n1, "eth0/4") && get_node_if_by_name(n1, "eth0/0"));
    node_t *n2 = get_node_by_node_name(graph, "R1_re");
    assert(n2 && get_node_if_by_name(n2, "eth0/1") && get_node_if_by_name(n2, "eth0/2"));
    node_t *n3 = get_node_by_node_name(graph, "R2_re");
    //&& get_node_if_by_name(n3, "eth0/3") && get_node_if_by_name(n3, "eth0/4"
    assert(n3 && get_node_if_by_name(n3, "eth0/5"));
    
    /* Some negative test cases */
    assert(!get_node_if_by_name(n3, "eth0/4"));
    assert(!get_node_if_by_name(n2, "eth0/5"));
    assert(!get_node_by_node_name(graph, "R3_re"));
    assert(!get_node_by_node_name(graph, "R4_re"));
    
    printf("get node by functions work!\n");
}
